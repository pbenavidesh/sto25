...
ets_trend <- gas_train |> 
  model(
    holt = ETS(Gas ~ error("A") + trend("A") + season("N")),
    tend_amort = ETS(Gas ~ error("A") + trend("Ad", phi = 0.98) + season("N"))
  )
...
STLF_holt <- decomposition_model(
      ...
      ETS(season_adjust ~ error("A") + trend("Ad", phi = 0.8) + season("N")),
      SNAIVE(season_year)
    )
...
gas_fit_todos <- gas_fit |> 
  cross_join(hw) |> 
  mutate(
    snaive_stlf = (snaive + stlf)/2,
    snaive_hwbc = (snaive + hw_boxcox)/2,
    todos = (snaive + stlf + stlf_holt + hw + hw_boxcox)/5
  )
