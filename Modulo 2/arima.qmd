...
noruega_fit <- noruega |> 
  model(
    dayanni  = ARIMA(box_cox(GDP, noruega_lambda) ~ pdq(2,2,2) + PDQ(0,0,0)),
    dayanni2 = ARIMA(box_cox(GDP, noruega_lambda) ~ pdq(3,2,2) + PDQ(0,0,0)),
    paola    = ARIMA(box_cox(GDP, noruega_lambda) ~ pdq(1,2,1) + PDQ(0,0,0)),
    blas     = ARIMA(box_cox(GDP, noruega_lambda) ~ pdq(2,1,1) + PDQ(0,0,0)),
    fanny    = ARIMA(box_cox(GDP, noruega_lambda) ~ pdq(1,1,1) + PDQ(0,0,0)),
    pablo    = ARIMA(box_cox(GDP, noruega_lambda) ~ PDQ(0,0,0)),
    ets      = ETS(box_cox(GDP, noruega_lambda))
  )
...
noruega |> 
  model(
    pablo  = ARIMA(box_cox(GDP, noruega_lambda) ~ PDQ(0,0,0), stepwise = FALSE, approximation = FALSE)
  )
tictoc::toc()
...
noruega |> 
  model(
    pablo2  = ARIMA(box_cox(GDP, noruega_lambda) ~ pdq(p = 0:3, d = 1) + PDQ(0,0,0))
  )
tictoc::toc()
```
...
h02_train |> 
  model(
    arima1 = ARIMA(log(Cost) ~ pdq(2,1,1) + PDQ(1,1,2)),
    arima2 = ARIMA(log(Cost) ~ pdq(2,1,2) + PDQ(1,1,2))
  )
```
